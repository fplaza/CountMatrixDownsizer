...
TEST_DIR=tests/input/7.too_many_threads
...
${COUNT_MATRIX_DOWNSIZER} --count-matrix ${TEST_DATA} --target-read-count ${TARGET_READ_COUNT} --num-repetitions ${NUM_REPETITIONS} --num-threads 10000 --downsized-matrix ${DOWNSIZED_MATRIX} --stats ${STATS}
