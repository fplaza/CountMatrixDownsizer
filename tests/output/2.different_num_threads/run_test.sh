...
TEST_DIR=tests/output/2.different_num_threads
...
${COUNT_MATRIX_DOWNSIZER} --count-matrix ${TEST_DATA} --target-read-count ${TARGET_READ_COUNT} --num-repetitions ${NUM_REPETITIONS} --num-threads ${NUM_THREADS} --downsized-matrix ${DOWNSIZED_MATRIX} --stats ${STATS}
